...
// Create
router.post('/', auth, validator(z.object({ title: z.string().min(1).max(120) })), async (req, res) => {
  try {
...
// Update
router.put('/:id', auth, validator(z.object({ title: z.string().min(1).max(120) })), async (req, res) => {
  try {
